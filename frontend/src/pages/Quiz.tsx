...
type QuizStatus = 'respondendo' | 'concluido'
...
export default function Quiz({ usuarioId, refazendo = false, onConcluido }: QuizProps) {
    ...
    return (
      <div className="quiz-container">
        <div className="quiz-concluido">
          <span className="quiz-concluido-icone">✓</span>
          <h2>{refazendo ? 'Perfil atualizado!' : 'Quiz concluído!'}</h2>
          <p>
            {refazendo
              ? 'Suas preferências foram atualizadas. Veja os novos destinos recomendados.'
              : 'Seu perfil de viajante foi criado. Confira os destinos que combinam com você!'}
          </p>
          <div className="quiz-concluido-acoes">
            <button className="btn-primario" onClick={onConcluido}>
              Ver destinos
            </button>
  ...
  return (
    <div className="quiz-container">
      <h1 className="quiz-title">
        {refazendo ? 'Refazer Quiz' : 'Quiz de Estilo de Viagem'}
      </h1>
      ...
      <div className="quiz-nav">
        ...
        >
          {enviando ? 'Salvando...' : ehUltimo ? 'Concluir' : 'Próxima'}
        </button>
      </div>
    </div>
  )
